    ...
    beforeMount(el: any, binding: any, vnode: any) {
      ...
      'keydown|keyup'.split('|').forEach(function (eitem) {
        el.addEventListener(eitem, (event: any) => {
          if (event.type === 'keyup') {
            if (event.key !== 'Backspace' && typeof binding.value.run === 'function') binding.value.run(event); // is typing running callback
            clearTimeout(inputTimer);
            inputTimer = setTimeout(() => {
              if(typeof binding.value.finish === 'function') binding.value.finish(event);
